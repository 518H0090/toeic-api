...
public class ParagraphProducer {
    ...
    private final KafkaTemplate<String, ParagraphExternal> kafkaTemplate;
    private final ConcurrentHashMap<String, CompletableFuture<String>> pendingResponses = new ConcurrentHashMap<>();

    public CompletableFuture<String> send(ParagraphExternal exam) {
        String correlationId = UUID.randomUUID().toString();
        CompletableFuture<String> futureResponse = new CompletableFuture<>();
        pendingResponses.put(correlationId, futureResponse);

        Message<ParagraphExternal> message = MessageBuilder
                .withPayload(exam)
                .setHeader(KafkaHeaders.TOPIC, "paragraph")
    ...
    @KafkaListener(topics = "paragraph-response", groupId = "demoGroup")
